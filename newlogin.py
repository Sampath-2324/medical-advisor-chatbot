...
def set_background():
    img = Image.open("background_image.jpg")  # Replace with your image file
    img = img.resize((300, 250), Image.ANTIALIAS)  # Resize the image to fit the window
...
def main_account_screen(frmmain):
    ...
    login_img = Image.open("login_button.png")  # Replace with your login button image
    login_img = login_img.resize((100, 50), Image.ANTIALIAS)
    login_photo = ImageTk.PhotoImage(login_img)
    Button(main_screen, image=login_photo, command=login).pack()

    Label(main_screen, text="").pack()
    
    # Button with an image
    register_img = Image.open("register_button.png")  # Replace with your register button image
    register_img = register_img.resize((100, 50), Image.ANTIALIAS)
    register_photo = ImageTk.PhotoImage(register_img)
    Button(main_screen, image=register_photo, command=register).pack()
